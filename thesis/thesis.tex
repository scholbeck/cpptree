...
Most techniques such as CART create binary decision trees. In theory, a series of binary splits is able to represent the same data partitioning as a multiway split  \cite{hastie_elemstatlearn} (see Fig. \ref{fig:binary_vs_multiway}). However, due to greedy optimization, a multiway split is rarely discovered by binary splitting \cite{wang_multiway_splits}. Binary decision trees are subject to a high variance, as a decision higher up cascades through the entire tree and results in different splits lower down the tree \cite{hastie_elemstatlearn}. 
\begin{figure}
    \centering
