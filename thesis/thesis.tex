...
First, computations can be parallelized. After splitting a node, the computations in all child nodes are independent of each other and can run in parallel.
Another option is to divide the value range of continuous features into parallely explored intervals, e.g., quantiles.
...
Third, both model and objective can be computed incrementally, i.e., instead of recomputing the entire model and objective for each split, an update mechanism reduces computational costs.
\par
The third mechanism to speed up the training process, incremental computations, depends on the objective and node model types. In the following, incremental computations are described for both the SSE and linear model trees, which are both implemented in the software package. 


\subsection{CART}
CART is a commonly used decision tree variant \cite{hastie_elemstatlearn}, which does not train models inside the leaf nodes. For regression trees, CART minimizes the weighted sum of squared errors (i.e., the L2 loss) between the child node target values and the corresponding mean value. For classification trees, CART minimizes Gini impurity, which summarizes the distribution of relative target class frequencies. CART is restricted to binary splits. The tree is fully grown until a pre-specified minimum node size criterion would be violated when splitting further. The tree height is pruned post-hoc according to a complexity parameter to avoid overfitting, which is estimated via cross-validation. Missing values are handled via surrogate splits.
%\par
...
\par
Given a split which partitions a node into two subsets with $n_1$ and $n_2$ observations, and child node L2 losses $L_1$ and $L_2$. The left node L2 loss $L_1$ with $n_1$ observations and target vector $y_1$ can be decomposed as:
$$
...
M5 starts with growing a decision tree by selecting splits that maximize the reduction of target value standard deviation. It then adds linear regression models to the leaf nodes, and prunes the tree. 
\par
