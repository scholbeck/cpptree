...
Decision tree learning algorithms recursively partition the data. The default approach is through greedy optimization of an objective function, although recent attempts have tried global optimization routines \cite{norouzi_nongreedy_tree}.
...
Third, for each node there is a large number of split points to be evaluated. Instead of recomputing the objective for each split, computations may be sped up by an update mechanism. For hybrid trees, the leaf node model contains a predictive model. Recomputing the model for each split is infeasible in practice. One option is to use a heuristic to split the tree and train the leaf node model post-hoc, which may result in a bad tree fit. Another option is to update the node model with a shrinking subset of observations at each split, and using the model fit as a basis for splitting the tree.
Fourth, an intelligently constructed split point generator can suggest split points with a higher probability of improving the objective than other points.
...
$$
The TSS is used by algorithms such as CART. It is equivalent to the sum of squared errors (SSE), also referred to as the L2 loss, if the trained model corresponds to the mean target value:
\begin{align*}
...
M5 starts with growing a decision tree by selecting splits that minimize the standard deviation of target values. It then adds linear regression models to the leaf nodes, and prunes the tree. Training linear leaf node models, whereas the splits have been determined via reduction of target variance creates a discrepancy in model fit.
\par
