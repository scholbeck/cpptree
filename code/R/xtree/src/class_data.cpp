...
std::array<std::vector<int>, 2> Data::getColTypesNumCateg() {
	int n_cols = this->ncols();
	std::vector<int> col_ix_num, col_ix_categ;
	for (int j = 1; j < n_cols; j++) {
		// exclude first column with ID
		if (this->getColTypes()[j] == "num") {
			col_ix_num.push_back(j);
		} else if (this->getColTypes()[j] == "categ") {
			col_ix_categ.push_back(j);
		}	
	}
	std::array<std::vector<int>, 2> coltypes;
	coltypes[0] = col_ix_num;
	coltypes[1] = col_ix_categ;
	return coltypes;
}
...
void Data::createObservationIDs() {
	this->observations = initVectorSeq(0, this->nrows() - 1);
}

std::vector<int> Data::getObservationIDs() {
