...
void ModelAverage::update(std::vector<double> observation, int target, char setdiff) {
	if (setdiff == '+') {
		this->cumsum_target += observation[target];
		this->n += 1;
...
void ModelLinearRegression::update(std::vector<double> observation, int target, char setdiff) {	
