...
double ObjectiveSSE::compute(Data* data, std::vector<int> observations, Model* mod) {
	int n = observations.size();
	double cumsum = 0;
	double target, pred;
	std::vector<double> singleobs;
	for (int i = 0; i < n; ++i) {
		singleobs = data->row(observations[i]);
		pred = mod->predictSingle(singleobs);
		target = data->elem(observations[i], data->getTargetIndex());
...
void ObjectiveSSE::update(Data* data, int childnode, std::array<std::vector<int>, 2> diff) {
	//if (!(diff[0].empty()) && diff[1].empty()) {
		...
		for (int i = 0; i < n_setplus; ++i) {
			...
			this->models[childnode]->update(observation, data->getTargetIndex(), '+');
...
double ObjectiveGini::compute(Data* data, std::vector<int> observations, Model* mod) {
	std::vector<double> target_obs = data->subsetRowsCol(observations, data->getTargetIndex());
