...
void SplitNum::computePartitionings(Data* data, std::vector<int> observations) {
	...
	for (int i = 0; i < n_obs; i++) {
		for (int j = 0; j < n_splits; j++) {
			if (data->selectObs(observations[i])[feature] <= split_values[j]) {
				split_multiway[j].push_back(observations[i]);
				// insert ID entry of observation;
				rightmost_node = false;
				break;
			}
		}
		if (rightmost_node == true) {
			split_multiway[n_splits].push_back(data->selectObs(observations[i])[0]);
		}
		rightmost_node = true;
