...
SplitGenerator* Factory::createSplitGenerator(Data* data, std::vector<int> obs, Arguments args) {
	SplitGenerator* g;
	if ((this->args.getAlgorithm() == "exhaustive") && (this->args.getMaxChildren() == 2)) {
		g = new SplitGeneratorBinExh(data, obs, args);
	} else if ((this->args.getAlgorithm() == "random") && (this->args.getMaxChildren() > 2)) {
		g = new SplitGeneratorMultRand(data, obs, args);
	}
	return g;
}
