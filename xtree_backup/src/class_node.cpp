...
Node::Node(std::string id, Tree* tree, std::vector<int> observations, std::string decision_rule) {
	this->tree = tree;
...
std::vector<Node*> Node::splitNode() {
	...
	SplitGenerator* split_generator = this->tree->getFactory().createSplitGenerator(this->tree->data, this->observations, this->tree->getArgs()); 
	...
	this->obj_val =  opt_obj_val = obj->compute(this->tree->data, this->observations, parent_mod);
	//Data* subset;
	if (!splits.empty()) {
		for (int i = 0; i < n_splits; ++i) {
			...
			if (i == 0) {
				...
				for (int j = 0; j < n_children; j++) {
					...
					diff = diffSet(splits[i]->splitted_obs[j], emptyvec);
					obj->update(this->tree->data, j, diff);
